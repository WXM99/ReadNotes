...
每个类一张表, 每张表拥有类的全部属性, 但是其ID在一个域内，该实现无法在父类级别上建立联系集。

### 2. Single table

![image-20190619232519576](review.assets/image-20190619232519576.png)

所有类塌缩成一个具有所有属性的表，没有了join的消耗，但是整张表会很大，粒度比较粗。

### 3. Joined table

![image-20190619234236526](review.assets/image-20190619234236526.png)

没每个类一张表维持pk可特殊属性, 另有一张表维护所有的pk和公共属性，在获取一条事务的所有属性时，需要join，在此上消耗较大。
